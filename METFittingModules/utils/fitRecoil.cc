...
void fitRecoil(TString Datafilename="/afs/cern.ch/user/g/gkrintir/github/HI/CMSSW_7_5_7_patch2/src/UserCode/diall/AnaResults.root",  // input ntuple for data
	       TString Bkgfilename="/afs/cern.ch/user/g/gkrintir/github/HI/CMSSW_7_5_7_patch2/src/UserCode/diall/out/AnaResults.root",  // input ntuple for bkg
	       ...
	       )
{
  ...
  TList * metPFRaw14 = dynamic_cast<TList*>(file->Get("metPFRaw"));
  TTree *treephys14 = dynamic_cast<TTree *>(metPFRaw14->FindObject("METPerformanceInf"));

  //Bkg.
  file = new TFile(Datafilename);
  if (file->IsZombie()) gSystem->Abort();
  TList * metPFRaw15 = dynamic_cast<TList*>(file->Get("metPFRaw"));
  TTree *treephys15 = dynamic_cast<TTree *>(metPFRaw15->FindObject("METPerformanceInf"));
  
  TH1F *histozll_pt = new TH1F("histozll_pt","histozll_pt",100,-400,400); // added later
  ...
  for(Int_t ibin=0; ibin<nbins; ibin++) {
    RooDataSet *data = new RooDataSet(Form("data_%d", ibin), "data", RooArgSet(uParaZllPt, PtZCands), Import(*treephys14), Cut(cut.data()));
    RooDataSet *bkg = new RooDataSet(Form("bkg_%d", ibin), "bkg", RooArgSet(uParaZllPt, PtZCands), Import(*treephys15), Cut(cut.data()));
